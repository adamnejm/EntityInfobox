...
-- check for CPPI and FPP
if not CPPI then
    print("EntityInfobox NOT LOADED: clientside Prop Protection with CPPI not found")
    return
elseif CPPI.GetName and CPPI.GetName() ~= "Falco's prop protection" then
    print("EntityInfobox NOT LOADED: please acquire Falco's Prop Protection")
    return
end

-- convars
local enabled        = CreateClientConVar("entityinfobox_enabled", "1", true, false, "Do enable the EntityInfobox")
...
if enabled:GetBool() and ConVarExists("FPP_PrivateSettings_HideOwner") then
...
hook.Add("HUDPaint", "EntityInfobox_HUDPaint", function()
    ...
    if traceEnt ~= ent or data.doUpdate then
        if traceEnt and traceEnt:IsValid() and not traceEnt:IsPlayer() then
    ...
    if ent and ent:IsValid() then updateDataDynamic(ent) end
